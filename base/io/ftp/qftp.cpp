...
QFtpCommand::QFtpCommand(QFtp::Command cmd, QStringList raw, const std::vector<unsigned char> &ba)
	: command(cmd), rawCmds(raw), is_ba(true)
{
	id = idCounter.fetchAndAddRelaxed(1);
	data.ba = new QByteArray((char*)ba.data(), (int)ba.size());
}
...
{
    commandSocket.close();
    emit connectState(QFtp::Disconnected);
}
...
int QFtp::put(const std::vector<unsigned char> &data, const QString &file, TransferType type)
{
	QStringList cmds;
	if (type == Binary)
		cmds << QLatin1String("TYPE I\r\n");
